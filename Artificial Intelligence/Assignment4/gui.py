...
def movingDrone(currentMap, path, controller, speed = 1,  markSeen = True):
    ...
    for i in range(len(path)):
        ...
        sensor_list = controller.get_sensor_list()
        for individual_sensor in sensor_list:
            positionX = individual_sensor.x_position
            positionY = individual_sensor.y_position
            screen.blit(sensor, (positionX * 20, positionY * 20))

            sensor_vision = individual_sensor.visible_squares
            for positions in sensor_vision:
                screen.blit(seen, (positions[0] * 20, positions[1] * 20))
